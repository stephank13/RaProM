...
###modified: Stephan Kral, Nov 2022
...
#    columns=list(' ')
    ...
    while line := f.readline():
        ...
        # read 64 following lines containing F
        Flines=[]
        for j in range(64):
            Fline=f.readline()
        ...
        if len(timeList)==0:            # if first time step
            ...
            if timeList[-1]<dat:        # if time vector is increasing
                timeList.append(dat)
            else:
                continue       

        # write all lines for one time step to corrected file if this time step does not contain errors
        f1.write(Mline)
        f1.write(Hline)
        f1.write(TFline)
        for j in range(64):
...
#     file_length = len(f.read().split('\n'))
#     #totallines=(file_length-7)/67   ### why -7 and not -1
#     totallines=(file_length-1)/67   ### why -7 and not -1
#     f.close()
...
#         line=f.readline()           # read lines starting with MRR
#         line2=line.strip()  
#         columns=line2.split()
#         if len(columns)==1:         # something unusal
#             longStr=2
...
print('The script generates netcdf files with the same name as the raw files\n')
...
##    print('altures',HIcolum2)
    ...
    if os.path.exists(filenameplot+'.nc'):
...
#    nc_times.description='Time in unix format'
    ...
    nc_Format_times.standard_name='time'
    nc_Format_times.long_name='time UTC'
    nc_Format_times.units='seconds since 1970-01-01'
    nc_Format_times.calendar='standard'
    # nc_Format_times.decription='time UTC'
    nc_Format_times.axis='T'
    nc_Format_times.coverage_content_type='coordinate'

    dataset.createDimension('Height',len(HIcolum2[1:]))
#    dataset.createDimension('levels',len(HIcolum2[1:]))
    nc_ranges_H=dataset.createVariable('Height','f',('Height',))
    nc_ranges_H.units = 'm'
    if np.isnan(h0_opt):
        nc_ranges_H.standard_name='height'
        nc_ranges_H.long_name = 'Heights in meters a.g.l.'
    else:
        nc_ranges_H.standard_name='altitude'
        nc_ranges_H.long_name = 'Heights in meters a.s.l.'
    nc_ranges_H.coverage_content_type='coordinate'
    nc_ranges_H.positive = 'up'
    ...
    if np.isnan(h0_opt):
        nc_ranges_H_PIA.standard_name='height'
        nc_ranges_H_PIA.long_name = 'Heights in meters a.g.l.'
    else:
        nc_ranges_H_PIA.standard_name='altitude'
        nc_ranges_H_PIA.long_name = 'Heights in meters a.s.l.'
    nc_ranges_H_PIA.coverage_content_type='coordinate'
    nc_ranges_H_PIA.positive = 'up'
    
    # dataset.createDimension('BB_Height',1)
    # nc_ranges_H_BB=dataset.createVariable('BB_Height','f',('BB_Height',))
    # nc_ranges_H_BB.units = 'm'
    # if np.isnan(h0_opt):
    #     nc_ranges_H_BB.standard_name='height'
    #     nc_ranges_H_BB.description = 'Heights in meters a.g.l.'
    # else:
    #     nc_ranges_H_BB.standard_name='altitude'
    #     nc_ranges_H_BB.description = 'Heights in meters a.s.l.'
    # nc_ranges_H_BB.coverage_content_type='coordinate'
    # nc_ranges_H_BB.positive = 'up'
        
    dataset.createDimension('DropSizeBin',len(D[0]))
    ncShape2D_D = ('Height','DropSizeBin',)
    # nc_ranges_DropSize=dataset.createVariable('DropSize','f',('DropSizeBin',))
    # nc_ranges_DropSize.long_name='drop size'
    # nc_ranges_DropSize.units = 'mm'
    # nc_ranges_DropSize.description = 'Size of the water hydrometeors'
    # nc_ranges_DropSize.coverage_content_type='coordinate'
    nc_ranges_DropSize=dataset.createVariable('DropSize','f',ncShape2D_D)
    nc_ranges_DropSize.long_name='hydrometeor drop size'
    nc_ranges_DropSize.units = 'mm'
    nc_ranges_DropSize.description = 'Size of the water hydrometeors'
    nc_ranges_DropSize.coverage_content_type='coordinate'
...
#                nc_times[Timecount:Timecount+1]=timeVec#add 1 second, beacuse the timestapmps from Mrr is the last second
                ...
                nc_nw[Timecount,:]=np.array(np.ma.masked_invalid(NW10),dtype='f')
                ...
                nc_N_daTH[Timecount,:,:]=np.array(np.ma.masked_invalid(NdE),dtype='f')
...
#                nc_N_da[Timecount,:]=np.array(np.ma.masked_invalid(DSD),dtype='f')
            ...
            break
        # read parameters from header line containing time stamp (first line)
        columns=line.split()
        if columns[0]!='MRR':
            # redirect to ???
        ...
        #Read the height parameters (second line from raw file)
        H=f.readline()
        H=H.strip()
        Hcolum=H.split()
        if Hcolum[0]!='H':
        ...
        for j in range(Number_bins):
            ...
            Data=Data.strip()
            Data=Data.split()
            if int(Data[0][1:3])!=j or Data[0][0]!='F':
                # redirect to ???
                print('wrong line found')
### insert a check for if int(Data[0][1:3])==j and Data[0][0]=='F' 
            Dades1=map(int,Data[1:len(Data)]) #extract the title for eac line F00, F01, etc
            Dades=np.fromiter(Dades1,dtype=np.int)
            FQ.append(Data[0])
            DataT.append(Dades)

        # convert F to spectra using TF
        Pot=[]
        for k in range(len(Harray)):#the result is amtrix corrected by transfer function and height, and dimension (LenHcolum)-1)
...
#            nc_times[Timecount:Timecount+1]=timeVec#add 1 second, beacuse the timestapmps from Mrr is the last second
            ...
            nc_ranges_DropSize[:]=np.array(np.ma.masked_invalid(D),dtype='f')
            ...
            ncShape3D = ('time_utc','Height','DropSizeBin',)
            if Timecount==0:
                ##################create netcdf############
                # idstr='GFI_MRR2_RaProM_'+str(IntTime)+'sec_'+unix2date(timeList[0]).strftime('%Y%m%d')
                # dataset.createDimension('chars',len(idstr))
                # #nc_id=dataset.createVariable('dataset_id', 'c', 'chars')
                # nc_id=dataset.createVariable('dataset_id', 'i')
                # #nc_id.long_name='dateset_id'
                # nc_id.cf_role='timeseries_id'
                # # nc_id=idstr
                # ncid=int(timeList[0]/3600/24)
                
                nc_w=dataset.createVariable('W','f',ncShape2D)
                nc_w.long_name='downward hydrometeor velocity'
                nc_w.description='hydrometeor fall speed/Doppler peak, with aliasing correction'
                nc_w.units='m s-1'
                # nc_w.positive='down'
                nc_sig=dataset.createVariable('sigW','f',ncShape2D)
                nc_sig.long_name='standard deviation of downward hydrometeor velocity'
                ...
                nc_sk.long_name='skewness of spectral reflectivity'
                ...
                nc_kur.long_name='kurtosis of the spectral reflectivity'
                ...
                nc_PIA.long_name='path integrated attenuation from liquid hydrometeors'
                ...
                nc_PIA_all.long_name='path integrated attenuation from all hydrometeors'
                ...
                nc_state.long_name='hydrometeor type'
                nc_state.description='type from hydrometeor as unknown (20), rain (10), drizzle (5), snow (-10), mixed (-15) and hail (-20) '
                nc_state.units=' '
                nc_LWC=dataset.createVariable('LWC','f',ncShape2D)
                nc_LWC.long_name='liquid water content'
                nc_LWC.units='g m-3'
                nc_RR=dataset.createVariable('RR','f',ncShape2D)
                nc_RR.standard_name='rainfall_rate'
                nc_RR.long_name='rain rate'
                nc_RR.units='mm h-1'
                nc_SnowR=dataset.createVariable('SR','f',ncShape2D)
                nc_SnowR.standard_name='lwe_snowfall_rate'
                nc_SnowR.long_name='liquid water equivalent snowfall rate'
                nc_SnowR.units='mm h-1'
                nc_Z_DA=dataset.createVariable('Z','f',ncShape2D)
                nc_Z_DA.long_name='radar reflectivity of liquid hydrometeors'
                nc_Z_DA.description='reflectivity considering only liquid drops'
                nc_Z_DA.units='dBZ'
                nc_Z_da=dataset.createVariable('Za','f',ncShape2D)
                nc_Z_da.long_name='attenuated radar reflectivity of liquid hydrometeors'
                nc_Z_da.description='atteunated reflectivity considering only liquid drops'
                nc_Z_da.units='dBZ'
                nc_Z_e=dataset.createVariable('Ze','f',ncShape2D)
                nc_Z_e.standard_name='equivalent_reflectivity_factor'
                nc_Z_e.long_name='equivalent radar reflectivity factor'
                nc_Z_e.units='dBZ'
                # nc_N_da=dataset.createVariable('logn','f',ncShape2D)
                # nc_N_da.description='Drop Size Distribution'
                # nc_N_da.description='Sum of Drop Size Distribution'
                # nc_N_da.units='log10(m^-3 mm^-1)'
                # nc_N_daTH=dataset.createVariable('N(D) in function of time and height','f',ncShape3D)
                # nc_N_daTH=dataset.createVariable('logN','f',ncShape3D)
                # nc_N_daTH.long_name='logarithmic spectral drop density'
                # nc_N_daTH.description='logarithmic drop size distribution as function of time and height, logN=NaN corresponds to N=0'
                # nc_N_daTH.units='log(m-3 mm-1)'
                nc_N_daTH=dataset.createVariable('N','f',ncShape3D)
                nc_N_daTH.long_name='spectral drop density'
                nc_N_daTH.description='drop size distribution as function of time and height'
                nc_N_daTH.units='m-3 mm-1'
                nc_SNR=dataset.createVariable('SNR','f',ncShape2D)
                nc_SNR.long_name='signal-to-noise ratio'
                nc_SNR.description='signal-to-noise ratio from signal without dealiasing'
                nc_SNR.units='dB'
                nc_Noi=dataset.createVariable('Noise','f',ncShape2D)
                nc_Noi.long_name='noise'
                nc_Noi.units='m-1'
                nc_nw=dataset.createVariable('Nw','f',ncShape2D)
                nc_nw.long_name='liquid water content normalized intercept'
                nc_nw.description='intercept parameter of the gamma distribution normalized to the liquid water content'
                nc_nw.units='m-3 mm-1'
                #nc_nw.units='log(m-3 mm-1)'
                nc_dm=dataset.createVariable('Dm','f',ncShape2D)
                nc_dm.long_name='mean hydrometeor diameter'
                nc_dm.description='mean mass-weighted hydrometeor diameter'
                nc_dm.units='mm'
                nc_bb_bot=dataset.createVariable('BB_bottom','f',ncShape2D_BB)
                nc_bb_bot.long_name='height from radar bright band bottom'
                if np.isnan(h0_opt):
                    nc_bb_bot.standard_name='height'
                    nc_bb_bot.description='height from radar bright band bottom in meters a.g.l.'
                else:
                    nc_bb_bot.standard_name='altitude'
                    nc_bb_bot.description='height from radar bright band bottom in meters a.s.l.'
                nc_bb_bot.units='m'
                nc_bb_bot.positive='up'
                nc_bb_top=dataset.createVariable('BB_top','f',ncShape2D_BB)
                nc_bb_top.long_name='height from radar bright band top'
                if np.isnan(h0_opt):
                    nc_bb_bot.standard_name='height'
                    nc_bb_top.description='height from radar bright band top in meters a.g.l.'
                else:
                    nc_bb_bot.standard_name='altitude'
                    nc_bb_top.description='height from radar bright band top in meters a.s.l.'
                nc_bb_top.units='m'
                nc_bb_top.positive='up'
...
#            nc_N_da[Timecount,:]=np.array(np.ma.masked_invalid(DSD),dtype='f')
    ...
    dataset.calibration_constant=str(CC)
    ...
    dataset.firmware_version=DVS
    dataset.instrument_serial_number=str(DSN)
#    dataset.instrument_bandwidth=str(BW)+' Hz'
    dataset.input_file_type=TypeFile
    dataset.instrument_altitude=str(h0_opt)+' m'
    dataset.averaging_period=str(IntTime)+' s'
    dataset.featureType ='timeSeriesProfile'
    dataset.geospatial_vertical_positive = 'up'
    dataset.geospatial_vertical_max=str(HIcolum2[-1])+' m'
    dataset.geospatial_vertical_min=str(HIcolum2[1])+' m'
    dataset.source = 'remote sensing observation' 
    dataset.standard_name_vocabulary = 'CF Standard Name Table v61'
    dataset.station_name='GFI_MRR2'
    dataset.time_coverage_start=unix2date(timeList[0]).strftime('%Y-%m-%dT%H:%M:%S%z')+TypeDate
    dataset.time_coverage_end=unix2date(timeList[-1]).strftime('%Y-%m-%dT%H:%M:%S')+TypeDate
    dataset.time_coverage_duration=unix2date(timeList[0]).strftime('%Y-%m-%dT%H:%M:%S')+TypeDate+'--'+unix2date(timeList[-1]).strftime('%Y-%m-%dT%H:%M:%S')+TypeDate
    dataset.time_coverage_resolution=str(IntTime)+' s'
    dataset.time_zone=TypeDate
    
    if ~np.isnan(Dm_2).all() and ~np.isnan(Nw_2).all():
        dm_ax,nw_ax,PrepTypeC=PrepType(Dm_2,Nw_2)

        # convert nw from log
        nw10_ax=10**np.array(nw_ax)
        nw10_ax[np.isnan(nw10_ax)] = 0
        ...
        nc_ranges_Dm.long_name='mean hydrometeor diameter'
        nc_ranges_Dm.description = 'mean diameter axes to precipitation type'
        nc_ranges_Dm.units = 'mm'
        nc_ranges_Dm.coverage_content_type='coordinate'
        nc_ranges_Nw.long_name='liquid water content normalized intercept'
        nc_ranges_Nw.description = 'intecept parameter axes to precipitation type'
        nc_ranges_Nw.units = 'm-3 mm-1'
        #nc_ranges_Nw.units = 'log(m-3 mm-1)'
        nc_ranges_Nw.coverage_content_type='coordinate'
        nc_ranges_Dm[:]=np.array(dm_ax,dtype='f4')
        # nc_ranges_Nw[:]=np.array(nw_ax,dtype='f4')
        nc_ranges_Nw[:]=np.array(nw10_ax,dtype='f4')
        nc_TypePrecipitation=dataset.createVariable('Precip_Type','f',('Dm_ax','Nw_ax',))
        nc_TypePrecipitation.long_name='preciptiation type'
